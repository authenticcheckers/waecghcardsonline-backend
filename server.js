// server.js — FINAL FIXED VERSION (NO SENDER ID REQUIRED)
...
// ❗ NO SENDER ID → Arkesel will assign default automatically
// ----------------------------------------------------------
...
async function initDb() {
  ...
  if (!admin) {
    ...
    if (!ADMIN_PASSWORD_HASH) {
      ...
      await db.run("INSERT INTO admin (username, password_hash) VALUES (?,?)", [
        ADMIN_USERNAME,
        ADMIN_PASSWORD_HASH,
      ]);
...
app.post("/admin/api/vouchers", ensureAdmin, async (req, res) => {
  ...
  if (!serial || !pin)
    return res.status(400).json({ message: "serial and pin required" });
  try {
  ...
  async (req, res) => {
    ...
    try {
      ...
      const insertStmt = await db.prepare(
        "INSERT OR IGNORE INTO vouchers (serial,pin,status) VALUES (?,?,?)"
      );
      let inserted = 0;
...
app.get("/admin/api/vouchers", ensureAdmin, async (req, res) => {
  ...
  try {
    const totalRow = await db.get(
      "SELECT COUNT(*) AS count FROM vouchers " + where,
      params
    );
...
app.get("/admin/api/stats", ensureAdmin, async (req, res) => {
  try {
    ...
    const unused = (
      await db.get("SELECT COUNT(*) AS c FROM vouchers WHERE status='unused'")
    ).c;
    const used = total - unused;
    const today = (
      await db.get(
        "SELECT COUNT(*) AS c FROM sales WHERE date(timestamp)=date('now')"
      )
    ).c;
...
app.post("/admin/api/resend-sms", ensureAdmin, async (req, res) => {
  ...
  try {
    const row = await db.get(
      "SELECT serial, pin, buyer FROM vouchers WHERE id=?",
      [id]
    );
    ...
    if (!phone)
      return res.status(400).json({ message: "No buyer phone/email recorded" });
    ...
    try {
      ...
      const fallbackUrl = `https://sms.arkesel.com/sms/api?action=send-sms&api_key=${encodeURIComponent(
        ARKESEL_API_KEY
      )}&to=${encodeURIComponent(phone)}&sms=${encodeURIComponent(
        `Your WASSCE voucher: Serial:${row.serial} PIN:${row.pin}`
      )}`;
...
app.post("/verify-payment", async (req, res) => {
  ...
  try {
    if (!PAYSTACK_SECRET_KEY)
      return res
        .status(500)
        .json({ success: false, message: "Server not configured with Paystack" });
    ...
    const row = await db.get(
      "SELECT id, serial, pin FROM vouchers WHERE status='unused' ORDER BY id ASC LIMIT 1"
    );
    if (!row) {
      await db.run("ROLLBACK");
      return res
        .status(500)
        .json({ success: false, message: "Out of vouchers" });
    }
