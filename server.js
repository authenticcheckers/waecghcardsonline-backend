...
app.post('/admin/api/vouchers/bulk', ensureAdmin, upload.single('file'), async (req, res) => {
  ...
  const records = csvParse(content, { skip_empty_lines: true, trim: true });
...
// verify-payment endpoint (frontend-friendly): verifies paystack and processes voucher
...
// paystack webhook (alternative/backup) - processes payment similar to verify-payment
